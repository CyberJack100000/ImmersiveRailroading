...
public class TileRail extends TileRailBase {
    ...
    private long monorailAnimationStart = 0;
    private static final long MONORAIL_ANIMATION_DURATION = 1000; // 1 second
    ...
    public void update() {
        ...
        // Handle monorail animation
        if (info.settings.getBoolean("monorail", false)) {
            if (SwitchUtil.isRailPowered(this)) {
                if (monorailAnimationStart == 0) {
                    monorailAnimationStart = System.currentTimeMillis(); // Start animation
                }
                long elapsed = System.currentTimeMillis() - monorailAnimationStart;

                if (elapsed >= MONORAIL_ANIMATION_DURATION) {
                    setSwitchState(SwitchState.MONORAIL_TURN);
                    monorailAnimationStart = 0; // Reset animation state
                }
            } else {
                monorailAnimationStart = 0; // Reset if unpowered
                setSwitchState(SwitchState.MONORAIL_STRAIGHT);
            }
        }
        ...
        if (getWorld().isServer && info != null && info.settings.type == TrackItems.TURNTABLE) {
            ...
            if (desiredPosition != info.tablePos) {
                ...
                this.markDirty();
                List<EntityCoupleableRollingStock> ents = getWorld().getEntities(
                        (EntityCoupleableRollingStock stock) -> stock.getPosition().distanceTo(new Vec3d(getPos())) < info.settings.length,
    ...
    public double percentFloating() {
        ...
        if (tracks == null) {
            tracks = (info.settings.type == TrackItems.SWITCH
                    ? info.withSettings(b -> b.type = TrackItems.STRAIGHT)
                    : info).getBuilder(getWorld(), new Vec3i(info.placementInfo.placementPosition).add(getPos()))
                    .getTracksForFloating();
            Vec3i offset = getPos().subtract(tracks.get(0).getPos());
            tracks = (info.settings.type == TrackItems.SWITCH
                    ? info.withSettings(b -> b.type = TrackItems.STRAIGHT)
                    : info).getBuilder(getWorld(), new Vec3i(info.placementInfo.placementPosition).add(getPos().add(offset)))
                    .getTracksForFloating();
        }
        ...
        public TagAccessor<List<ItemStack>> apply(Class<List<ItemStack>> type, String fieldName, TagField tag) {
            return new TagAccessor<>(
                    ...
                    nbt -> {
                        ...
                        if (nbt.hasKey("drops")) {
                            TagCompound dropNBT = nbt.getCompound("drops");
                            int count = dropNBT.getInteger("count");
                            for (int i = 0; i < count; i++) {
                                drops.add(ItemStack.fromTag(dropNBT.getCompound("drop_" + i)));
                            }
                        }
                        return drops;
                    }
