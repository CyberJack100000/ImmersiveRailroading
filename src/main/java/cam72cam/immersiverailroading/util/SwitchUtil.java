...
public class SwitchUtil {
    ...
    public static SwitchState getSwitchState(TileRail rail, Vec3d position) {
        ...
        // Add monorail-specific behavior
        boolean isMonorail = rail.info.settings.getBoolean("monorail", false);

        if (isMonorail) {
            if (isRailPowered(rail)) {
                if (animationStartTime == 0) {
                    animationStartTime = System.currentTimeMillis(); // Start animation
                }
                long elapsed = System.currentTimeMillis() - animationStartTime;
                if (elapsed < 1000) { // 1-second animation duration
                    return SwitchState.MONORAIL_MOVING;
                } else {
                    return SwitchState.MONORAIL_TURN; // Final state after animation
                }
            } else {
                animationStartTime = 0; // Reset animation
                return SwitchState.MONORAIL_STRAIGHT;
            }
        }
